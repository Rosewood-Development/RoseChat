...
public class CommandSocialSpy implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String cmd, String[] args) {
        if (sender instanceof Player) {
            ...
            if (player.hasPermission("rosechat.socialspy")) {
                if (!PlayerData.getPlayersUsingSocialSpy().contains(player.getUniqueId())) {
                    PlayerData.getPlayersUsingSocialSpy().add(player.getUniqueId());
                    player.sendMessage(Methods.format(Messages.getToggleSocialSpyOn()));
                } else {
                    PlayerData.getPlayersUsingSocialSpy().remove(player.getUniqueId());
                    player.sendMessage(Methods.format(Messages.getToggleSocialSpyOff()));
                }
