...
public class FromDiscordBungeeParser implements MessageParser<BaseComponent[]> {
    ...
    public MessageTokenizerResults<BaseComponent[]> parse(RoseMessage message, RosePlayer viewer, String format) {
        if (Settings.USE_MARKDOWN_FORMATTING.get()) {
            return MessageTokenizer.tokenizeAndCompose(message, viewer, format, MessageDirection.DISCORD_TO_MINECRAFT, TokenComposer::decorated,
