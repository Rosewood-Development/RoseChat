...
public class SocialSpyCommand extends AbstractCommand {
    ...
    private void sendToggleMessage(CommandSender sender, String[] args, boolean nowOn) {
        if (nowOn)
            this.getAPI().getLocaleManager().sendComponentMessage(sender, "command-socialspy-enabled", StringPlaceholders.of("type",
                    this.getAPI().getLocaleManager().getLocaleMessage("command-socialspy-" + args[0].toLowerCase())));
        else
            this.getAPI().getLocaleManager().sendComponentMessage(sender, "command-socialspy-disabled", StringPlaceholders.of("type",
                    this.getAPI().getLocaleManager().getLocaleMessage("command-socialspy-" + args[0].toLowerCase())));
    }
