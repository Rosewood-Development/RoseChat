...
import dev.rosewood.rosegarden.command.framework.Argument.SubCommandArgument;
...
public class GroupHelpCommand extends RoseChatCommand {

    private final BaseRoseCommand parent;

    public GroupHelpCommand(RosePlugin rosePlugin, BaseRoseCommand parent) {
        super(rosePlugin);
        this.parent = parent;
    }
    ...
    public static void displayHelpMessage(BaseRoseCommand parent, CommandContext context) {
        ...
        if (!(argument instanceof SubCommandArgument subCommandArgument))
        ...
        for (RoseCommand command : subCommandArgument.subCommands()) {
            ...
            localeManager.sendSimpleCommandMessage(context.getSender(),
                    "command-gc-help-list-description",
                    stringPlaceholders);
        }
    }
